...
public class GeneralSpecification {
    ...
    public static Specification<Model> mainFilters(ParamsDTO paramsDTO) {
        return (root, query, cb) -> {
            ...
            // фильтрация
            if (paramsDTO.getCountry() != null ) list.add(cb.equal(root.get("appliance").get("producerCountry").get("name"), paramsDTO.getCountry()));
            if (paramsDTO.getCompany() != null ) list.add(cb.equal(root.get("appliance").get("producerCompany").get("name"), paramsDTO.getCompany()));
            if (paramsDTO.getOnline() != null ) list.add(cb.equal(root.get("appliance").get("onlineOrder"), paramsDTO.getOnline()));
